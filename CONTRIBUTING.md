...
**标签约定**：主任务（实验一~五、期末项目）内的修改用 `enhancement` / `bug` / `documentation`；**四个主任务之外的修改**（功能增强、体验优化等）同样必须先建 issue，打 **`extension`** 标签。
...
- **每个任务（实验）实现完成并合入 main 后打 tag 并关闭 issue**：`lab1`、`lab2`…（老师要求所有实验在一个程序里，tag 即分任务提交节点）。**四个任务最后统一验收**（老师确认，非逐个验收）；统一验收通过后源码+可运行文件打包上传超星
